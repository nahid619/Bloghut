...
if(isset($_POST['submit']))
{
    ...
    if(!$firstname)
    {
        $_SESSION['adduser'] = "Please enter your Firstname";
    }
    else if(!$lastname)
    {
        $_SESSION['adduser'] = "Please enter your Lastname";
    }
    else if(!$username)
    {
 ...
 //   else if(!$is_admin)
   // {
    //    $_SESSION['adduser'] = "Please select user role";
    //}
    else if(strlen($createpassword)<8 || strlen($confirmpassword)<8)
    {
        $_SESSION['adduser'] = "Re-enter password.(Minimum 8 character)";
    }
    else if(!$avatar['name'])
    {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                $_SESSION['adduser']="username or email exists";
            }
            else
            {
                //work on avatar
                ...
                if(in_array($extension,$allowed_image))
                {
                    ...
                    {
                        $_SESSION['adduser'] = "Filesize is too large should be less than 1mb.";
                    }
                }
                else
                {
    ...
    else
    {
        $inset_user_query = "INSERT INTO users (firstname, lastname, username, email, password, avatar, is_admin) 
                        VALUES('$firstname','$lastname', '$username', '$email', '$password', '$avatar_name','$is_admin' )";
        ...
        if(!mysqli_errno($con))
        {
            $_SESSION['adduser-success'] = "New user $firstname $lastname added successfully.";
